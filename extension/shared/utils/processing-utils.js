...
export function buildTrackLabel(track, type, videoType) {
    if (type === 'video') {
        let resolution, standardizedRes, fps, fileSizeBytes, codecs;

        if (videoType === 'hls' && track.metaJS) {
            // HLS video track structure
            resolution = track.metaJS.resolution || null; // width x height format
            standardizedRes = track.metaJS.standardizedResolution || null;
            ...
            resolution = track.resolution || null; // width x height format
            standardizedRes = track.standardizedResolution || null;
            ...
            resolution = (track.metaFFprobe?.width && track.metaFFprobe?.height) ?
                `${track.metaFFprobe.width}x${track.metaFFprobe.height}` : null;
            standardizedRes = track.standardizedResolution || null;
        ...
        const formattedResolution = formatResolutionWithFps(standardizedRes, fps);
        const formattedSize = fileSizeBytes ? formatSize(fileSizeBytes) : null;

        return [formattedResolution, formattedSize, resolution, codecs]
            .filter(Boolean)
